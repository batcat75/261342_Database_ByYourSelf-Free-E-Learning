    ...
    <div id="wrapper">
		...
		<?php
			...
			if($tableName=='ENROLLMENT'){
				$s = oci_parse($c, "select mem_id, course_id, flag_pass, to_char(date_enroll,'yyyy/mm/dd') as DATE_ENROLL
									from enrollment 
									where ".$string." = ".$_GET["value_key1"]."
									and ".$string2." = ".$_GET["value_key2"]."");
			}else if($tableName=='TEST_HISTORY'){
				$s = oci_parse($c, "select test_id, mem_id, course_id, to_char(test_date,'yyyy/mm/dd') as TEST_DATE,score
									from test_history
									where ".$string." = ".$_GET["value_key1"]."");
			}else if($value_key2== 'NO_DATA'){
				$s = oci_parse($c, "select * from ".$_GET["table"]." 
									where ".$string." = ".$_GET["value_key1"]."");
			}else{
				$s = oci_parse($c, "select * from ".$_GET["table"]." 
		...
		?>
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"><?php echo $tableName?> : Edit </h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <?php echo "<i class=\"fa fa-pencil-square fa-fw\"></i>Edit Data Below";?>	
                        </div>
                        <!-- /.panel-heading -->
                        <div class="panel-body">
                            <form id="addForm" method="post" action="editRecordSave.php" class="form-horizontal">
							<fieldset>
								<?php
								...
								if($Num_fk>0){
									...
									for($k = 0;$k<$Num_fk;$k++ ){
										if($colname == $fk_colName[$k]){
											$have_fk=1;
											echo "<select class=\"form-control\" name=text".$i.">";
											...
											if($tableName !='ANSWER'){
												while($row_reference = oci_fetch_array($fk_reference)) {
													echo "<option ";
													if($row_reference[0] == $row[$i]){
														echo " selected ";
													}
													echo " value=\"".$row_reference[0]."\">".$row_reference[0]." --------- ".$row_reference[1]."</option>";
												}
											}else{
												while($row_reference = oci_fetch_array($fk_reference)) {
													echo "<option ";
													...
													echo " value=\"".$row_reference[0]."\">".$row_reference[0]." --------- ".$row_reference[2]."</option>";
												}
											}
											echo "</select>";
										}
									...
									?>
										<input type="date" class="form-control" name=text<?php echo "$i";?>  value="<?php echo $newformat;?>"  >
									<?php	
									}else if($have_fk==0){?>
										<input type="text" class="form-control" name=text<?php echo "$i";?>  value="<?php echo $value;?>"  >
									<?php
									}
								}else{
								?>
								...
								echo "<input type ='hidden' value='".$_GET["table"]."' name='table'>";
								echo "<input type ='hidden' value='".$_GET["value_key1"]."' name='value_key1'>";
								echo "<input type ='hidden' value='".$value_key2."' name='value_key2'>";
							?>
